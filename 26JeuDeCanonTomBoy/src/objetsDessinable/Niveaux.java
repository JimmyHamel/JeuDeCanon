...
public class Niveaux implements Dessinable {
	...
	private Area areaPlayer;
	private double kRessort = 100000;
	 ...
	 *
	 * @param coefTraine Le coefficient de trainée. Ce coefficient n'a pas de grandeur
	 *            physique.
	 * @param masseAir La masse volumique de l'air dans lequel la balle se déplacera
	 *            en kg/m<sup>3</sup>.
	 * @param qualite Un nombre entre 1 et 5 qui démontre la qualité du char où 1
	 ...
	 * @param nom Le nom du niveau choisi.
	 */
	...
	public Niveaux(double longueurMonde, double hauteurMonde, double g, double coefTraine, double masseAir, int qualite,
		...
		this.player = new Joueur(20, 5.0, this.longueurJoueur, this.largeurJoueur, coefTraine, masseAir, qualite,
				champMagnetique, chargeEnnemie, longueurMonde);
	...
	public void mouvementJoueur(String mouvement) {
		if (eulerI)
			this.player.eulerI(g, deltaT, this.kRessort, mouvement);
		else if (euler)
			this.player.euler(g, deltaT, this.kRessort, mouvement);
		else if (rk4)
			this.player.rk4(g, deltaT, this.kRessort, mouvement);
		if(mouvement.equals("ressortDroit")){
			kRessort+=20;
			ressortDroit.contract("right", this.deltaT);
		} else if(mouvement.equals("ressortGauche")){
			kRessort+=20;
			ressortGauche.contract("left", this.deltaT);
		}
	}
...
//			this.player.rk4(this.g, this.deltaT, this.ressortDroit.getK(), "ressortGauche");
//			System.out.println("HELLO");
//		}
//
//		if (playerArea.intersects(this.ressortDroit.getBoiteDeCollision())) {
//			this.player.rk4(this.g, this.deltaT, this.ressortDroit.getK(), "ressortDroit");
//		}
	...
	/**
	 * Méthode qui verifie si la balle a fait une colision avec un ennemi. S'il
	 * y a collision, l'ennemi touché sera éliminéé (retiré de la liste
	 * d'ennemi). Cette méthode sera appelée par la méthode testCollision() de
	 * cette même classe.
	 * 
	 * @param balleArea L'aire de la balle que le char d'assaut tire.
	 */
	 ...
	 * 
	 * @param g La valeur de la constante gravitationelle en m/s<sup>2</sup>
	 */
	...
	public Ressort getRessortDroit() {
		return ressortDroit;
	}

	public void setRessortDroit(Ressort ressortDroit) {
		this.ressortDroit = ressortDroit;
	}

	public Ressort getRessortGauche() {
		return ressortGauche;
	}

	public void setRessortGauche(Ressort ressortGauche) {
		this.ressortGauche = ressortGauche;
	}
	public boolean isRessortGaucheIntersects(){
		this.areaPlayer = new Area(player.getBoiteDeCollision());
		if(ressortGauche.equals(null))
			return false;
		else
			return areaPlayer.intersects(this.ressortGauche.getBoiteDeCollision());
	}
	public boolean isRessortDroitIntersects(){
		this.areaPlayer = new Area(player.getBoiteDeCollision());
		if(ressortDroit.equals(null))
			return false;
		else
			return areaPlayer.intersects(ressortDroit.getBoiteDeCollision());
	}
